...
def get_data(config):
    ...
    # compute render poses，render_pose会返回theta固定，angle在范围内插值的一系列位置
    N = 40
...
def build_models(config, disc=True):
    ...
    # 1是包含nerf模型和一些参数的字典，2是1更改了部分参数的字典，3是nerf模型的参数字典，4是nerf模型的层名称和参数字典
